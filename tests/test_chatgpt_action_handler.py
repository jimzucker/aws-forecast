"""Unit tests for chatgpt_action_handler.lambda_handler."""
import json
import unittest
from unittest.mock import patch

import chatgpt_action_handler as handler
...
def _seed_secret(session, token=VALID_TOKEN):
    session._clients["secretsmanager"].get_secret_value.return_value = {
        "SecretString": json.dumps({"chatgpt_bearer_token": token})
    }
...
class HandlerAuthTests(unittest.TestCase):
    ...
    def test_malformed_authorization_returns_401(self):
        with _patch_session(self.session):
            response = handler.lambda_handler(
                _make_event(authorization="Token xyz"), None
            )
        self.assertEqual(response["statusCode"], 401)
    ...
    def test_wrong_bearer_returns_401(self):
        with _patch_session(self.session):
            response = handler.lambda_handler(
                _make_event(authorization="Bearer wrong"), None
            )
        self.assertEqual(response["statusCode"], 401)
...
class HandlerSuccessTests(unittest.TestCase):
    ...
    def test_total_row_maps_to_top_level(self):
        with _patch_session(self.session), \
             patch.object(handler.get_forecast, "calc_forecast", return_value=CALC_ROWS):
            response = handler.lambda_handler(
                _make_event(authorization=f"Bearer {VALID_TOKEN}"), None
    ...
    def test_accounts_have_required_keys(self):
        with _patch_session(self.session), \
             patch.object(handler.get_forecast, "calc_forecast", return_value=CALC_ROWS):
            response = handler.lambda_handler(
                _make_event(authorization=f"Bearer {VALID_TOKEN}"), None
    ...
    def test_no_total_row_falls_back_to_zero(self):
...
class HandlerFailureTests(unittest.TestCase):
    ...
    def test_calc_forecast_raises_returns_500(self):
        ...
        self.assertEqual(
            ...
            {"error": "Cost Explorer unavailable"},
        )
        self.assertNotIn("ce down", response["body"])
...
class HandlerSecretRotationTests(unittest.TestCase):
    def test_rotated_secret_is_picked_up_immediately(self):
        session = make_mock_boto3_session()
        sm = session._clients["secretsmanager"]

        sm.get_secret_value.return_value = {
            "SecretString": json.dumps({"chatgpt_bearer_token": "old"})
        }
        ...
        sm.get_secret_value.return_value = {
            "SecretString": json.dumps({"chatgpt_bearer_token": "new"})
        }
        with _patch_session(session), \
             patch.object(handler.get_forecast, "calc_forecast", return_value=CALC_ROWS):

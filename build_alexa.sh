#!/usr/bin/env bash
# Build and upload the Alexa skill Lambda zip.
#
# Required env vars:
...
#   ZIP_KEY         S3 key (default: alexa_handler.zip).
set -euo pipefail
...
ZIP_KEY="${ZIP_KEY:-alexa_handler.zip}"
...
cp get_forecast.py alexa_handler.py target/build/
